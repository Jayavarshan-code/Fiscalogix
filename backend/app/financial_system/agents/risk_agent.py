...
class RiskAgent(BaseAgent):
    ...
    async def run_logic(
        ...
        # Row enrichments returned for fan-in — agent no longer mutates ctx.data rows.
        row_enrichments = [
            {
                "risk_score":      risk_outputs[i]["score"],
                "risk_confidence": risk_outputs[i]["confidence"],
                "risk_drivers":    risk_outputs[i].get("drivers", []),
            }
